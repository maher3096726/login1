...
from dotenv import load_dotenv  # ➕ إضافة مكتبة البيئة
...
app.secret_key = os.environ.get("SECRET_KEY", "hims_secret_key_2024")
app.config['UPLOAD_FOLDER'] = 'static/uploads'
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
CORS(app)

# ===================== Firebase (من متغير البيئة) =====================
FIREBASE_URL = os.environ.get("MY_FIREBASE_LINK")

# التحقق من وجود رابط Firebase
if not FIREBASE_URL:
    print("⚠️ تحذير: MY_FIREBASE_LINK غير موجود في ملف .env")
    print("يرجى إنشاء ملف .env وإضافة الرابط فيه")

def save_to_firebase(student_data, image_path_card=None, image_path_id=None):
    """حفظ البيانات في Firebase"""
    if not FIREBASE_URL:
        print("❌ رابط Firebase غير متوفر، يتم الحفظ محلياً")
        return save_locally(student_data)
...
def save_locally(data):
    ...
    try:
        ...
        print(f"✅ تم الحفظ محلياً في {filename}")
        return True
    except Exception as e:
        print(f"❌ فشل الحفظ المحلي: {e}")
        return False
...
if __name__ == '__main__':
    # قراءة إعدادات التشغيل من متغيرات البيئة
    ...
    print("🚀 تشغيل التطبيق")
    print(f"📍 المنفذ: {port}")
    print(f"🔧 وضع التصحيح: {debug}")
    print(f"🔥 رابط Firebase: {FIREBASE_URL if FIREBASE_URL else '❌ غير محدد'}")
    print("=" * 50)
